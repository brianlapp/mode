...
def main():
    ...
    # Today's Optimization Checklist (Future: Mike can add via Slack)
    st.header("📋 Today's Optimization Checklist")
    
    # Current manual checklist (Future: Slack-updatable)
    checklist_col1, checklist_col2 = st.columns(2)
    
    with checklist_col1:
        st.subheader("🎯 Mike's Priorities")
        st.checkbox("Review audit findings", value=False)
        st.checkbox("Approve visual strategy direction", value=False) 
        st.checkbox("Share current CPL data", value=False)
        st.checkbox("Set Week 1 budget/targets", value=False)
        
        st.markdown("**💡 Future: Mike can add tasks via Slack agent!**")
    
    with checklist_col2:
        st.subheader("⚡ Your Tasks (Night Shift)")
        st.checkbox("Source lifestyle visuals", value=False)
        st.checkbox("Write USA psychology headlines", value=False)
        st.checkbox("Prepare A/B test variations", value=False) 
        st.checkbox("Update Mike on progress", value=False)
        
        # Add task input for future Slack integration
